...
import com.alexdyysp.ReactorExampleApplication;
    ...
    private static void setupDatabase() {
        ...
        try (final Session session = cluster.connect()) {
            ...
            session.execute("CREATE TABLE blogs.blogpost (" +
                ...
                "body text" +
                ");");
            // insert into blogpost (id,author,body,title) VALUES(now(),'dyy','first test blog can be read','first test blog');
        } catch (Exception e){
            log.error("Cassandra has the blogs", e);
        }
    }
}
